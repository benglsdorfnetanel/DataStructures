...
namespace DS
{
    public class BST<T> where T : IComparable<T>
    {
        ...
        public void FindBestMatch(T data, out Node node)
        {
            ...
            {
                if (root == null) return root; // base case tree empty
                if (root.data.CompareTo(data) < 0) return FindBestMatch(root.right); // if smaller go right
                else if (root.data.CompareTo(data) == 0) return root; // if equals return this node
                else
                { // it is smaller than current node which means we can either reuturn this node or this node left sub tree has better match
